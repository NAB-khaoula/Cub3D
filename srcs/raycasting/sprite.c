...
/*   Updated: 2020/11/18 10:31:12 by knabouss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	gnrl->sprite.drawstarty = -gnrl->sprite.spriteheight
	/ 2 + gnrl->map.resol_h / 2;
	if (gnrl->sprite.drawstarty < 0)
		gnrl->sprite.drawstarty = 0;
	gnrl->sprite.drawendy = gnrl->sprite.spriteheight
	/ 2 + gnrl->map.resol_h / 2;
...
{
	gnrl->sprite.spritewidth = abs((int)(gnrl->map.resol_h
	/ gnrl->sprite.transform_y));
	gnrl->sprite.drawstartx = -gnrl->sprite.spritewidth
	/ 2 + gnrl->sprite.spritescreenx;
	if (gnrl->sprite.drawstartx < 0)
		gnrl->sprite.drawstartx = 0;
	gnrl->sprite.drawendx = gnrl->sprite.spritewidth
	/ 2 + gnrl->sprite.spritescreenx;
...
void	draw_vert_stripes(t_struct *gnrl)
{
	...
	{
		gnrl->texx = (int)(256 * (gnrl->sprite.stripe -
		(-gnrl->sprite.spritewidth / 2 + gnrl->sprite.spritescreenx))
		* gnrl->sprite.spr_w / gnrl->sprite.spritewidth) / 256;
		if (gnrl->sprite.transform_y > 0 && gnrl->sprite.stripe > 0
		&& gnrl->sprite.stripe < gnrl->map.resol_w
		&& gnrl->sprite.transform_y < gnrl->sprite.z_buffer
		[gnrl->sprite.stripe])
		{
			j = gnrl->sprite.drawstarty - 1;
			while (++j < gnrl->sprite.drawendy)
				fill_img_sprt(gnrl, j, d);
		}
	}
}
